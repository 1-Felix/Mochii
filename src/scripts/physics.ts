...
export function createMochi(x: number, y: number, tier: number): Mochi {
  ...
  // Cross springs (diameter springs - resist bell curve deformation)
  for (let i = 0; i < numPoints / 2; i++) {
    ...
    springs.push({
      ...
      restLength: Math.sqrt(dx * dx + dy * dy),
      stiffness: 0.35, // Stronger to prevent bell curve shape
    });
  }
...
export function updateMochi(
  ...
  // Spring forces with viscous damping (scaled by dt for frame-rate independence)
  ...
  // Tighter constraints to prevent bell-curve shape (sides collapsing while middle bulges)
  ...
  // Maximum radius prevents elongation - keep mochi more uniformly round
  ...
  // First three tiers should squish less visually since they're more resistant
  const maxSquish = mochi.tier <= 2 ? 0.3 : mochi.tier <= 4 ? 0.5 : 0.6;
  mochi.squishAmount = Math.min(mochi.squishAmount, maxSquish);

  // Ensure radius never goes below threshold (prevents face from being too small)
  ...
  if (mochi.hasLanded && mochi.settleTimer <= 0 && avgVelocity < SLEEP_VELOCITY_THRESHOLD && !mochi.grabbed) {
    for (const p of points) {
      p.vx = 0;
      p.vy = 0;
    }
...
export function canMerge(m1: Mochi, m2: Mochi): boolean {
  ...
  if (m1.tier >= mochiTiers.length - 1) return false; // Can't merge max tier
